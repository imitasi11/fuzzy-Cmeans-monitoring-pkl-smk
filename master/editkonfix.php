...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE konfix SET  Iterasi=%s, Bobot=%s, Eps=%s WHERE Kode=%s",
                       GetSQLValueString($_POST['iterasi'], "double"),
					   GetSQLValueString($_POST['bobot'], "double"),
					   GetSQLValueString($_POST['eps'], "double"),
					   GetSQLValueString($_POST['kdb'], "int"));

  mysql_select_db($database_RON, $RON);
  $Result1 = mysql_query($updateSQL, $RON) or die(mysql_error());

  $updateGoTo = "dconfix.php?nama=&submit=cari";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_RON, $RON);
$query_Recordset1 = sprintf("SELECT * FROM konfix WHERE Kode = %s", GetSQLValueString($colname_Recordset1, "int"));
...
<div id="main_content">
  <div id="content2">
    <div class="post">
      <h2 class="title"><a href="#">Edit Data Konfigurasi</a></h2>
      <hr />
          ...
          <form action="<?php echo $editFormAction; ?>" method="post" id="form2">
            <table align="center">
              <tr valign="baseline">
                ...
                <td><input name="iterasi2" type="double" value="<?php echo ($row_Recordset1['Iterasi']); ?>" size="32"  /></td>
              </tr>
              <tr valign="baseline">
                <td align="right">Bobot:</td>
                <td><input type="double" name="bobot2" value="<?php echo $row_Recordset1['Bobot']; ?>" size="20" /></td>
              </tr>
              <tr valign="baseline">
                <td align="right">Epsilon:</td>
                <td><input name="eps2" type="double" value="<?php echo ($row_Recordset1['Eps']); ?>" size="32"  /></td>
              </tr>
              <tr valign="baseline">
                <td align="right">&nbsp;</td>
            ...
            <input type="hidden" name="kdb2" value="<?php echo $row_Recordset1['Kode']; ?>" />
          </form>
        <p>&nbsp;</p>
      </div>
    </div>
